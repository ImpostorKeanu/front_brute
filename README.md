...
Domain fronting is a popular means of hiding the destination of a C2 server/network by exploiting insecure functioning of CDN nodes, which often use the value of the host header embedded in an HTTP request as the FQDN value as they proxy on behalf of CDN consumers. This is achieved by using a RAT supporing HTTP communication channels that can be configured with a custom host header, such as Meterpreter or Beacon.
